...
public final class JDOClassDescriptorFactory {
  ...
  /**
   * Creates a new {@link JDOClassDescriptorFactory} with the given configuration.
   *
   * @param config A {@link BuilderConfiguration} instance
   */
  ...
  public JClass createSource(final ClassInfo classInfo) {
    ...
    String descriptorClassName = getQualifiedJDODescriptorClassName(jClass.getName());
    JDODescriptorJClass classDesc = new JDODescriptorJClass(_config, descriptorClassName, jClass);
  ...
  /**
   * Returns the fully-qualified class name of the JDODescriptor to create. Given the
   * fully-qualified class name of the class we are creating a JDODescriptor for, return the correct
   * fully-qualified name for the JDODescriptor.
   *
   * @param name fully-qualified class name of the class we are describing
  ...
  private String getQualifiedJDODescriptorClassName(final String name) {
    ...
    if (descPackage != null && descPackage.length() > 0) {
      descPackage = descPackage + "." + JDOConstants.JDO_DESCRIPTOR_PACKAGE + ".";
    } else {
      descPackage = "";
    }
  ...
  /**
   * Returns the string which contains names of all FieldDescriptors for Fields that are not
   * identities. Names are separated by comma.
   * 
  ...
  /**
   * Returns the string which contains names of all FieldDescriptors for Fields that are identities.
  ...
  /**
   * Checks if ClassInfo has the JDOClassNature and if their required attributes are properly set.
   * 
   * @param cInfo the ClassInfo object
   * @return true when ClassInfo has a JDOClassNature with all required attributes properly set,
   *         false otherwise
   */
  private boolean checkClassInfoNature(final ClassInfo cInfo) {
    if (cInfo.hasNature(JDOClassInfoNature.class.getName())) {
      JDOClassInfoNature cNature = new JDOClassInfoNature(cInfo);
      if (cNature.getAccessMode() == null || cNature.getPrimaryKeys() == null
          || cNature.getPrimaryKeys().isEmpty() || cNature.getTableName() == null
  ...
  /**
   * Checks if FieldInfo has the XMLInfoNature and JDOFieldNature and if their required attributes
   * are properly set.
   * 
   * @param fInfo the FieldInfo object
   * @return true when FieldInfo has a XMLInfoNature and JDOFieldNature with all required attributes
  ...
  private boolean checkFieldInfoNatures(final FieldInfo fInfo) {
    ...
    if (fInfo.hasNature(JDOFieldInfoNature.class.getName())) {
      ...
      if (fNature.getColumnName() == null || fNature.getColumnName().length() == 0
  ...
  /**
   * Creates the source code of the FieldInfo part appeding it to the source code generated till
   * now.
   * 
  ...
  private JSourceCode createEntityFieldInfoPart(final FieldInfo fInfo, final JSourceCode jsc) {
    ...
    // TODO IS THERE A NEED TO CHECK THIS?!
    if ((className != null) && (className.length() > 0)) {
      jsc.add("Method " + name + "GetMethod = " + className + ".class.getMethod(\"get"
          + toUpperCaseFirstLetter(name) + "\", null);");
      jsc.add("Method " + name + "SetMethod = " + className + ".class.getMethod(\"set"
    ...
    jsc.add(name + "FieldDescr.setSetMethod(\"set" + toUpperCaseFirstLetter(name) + "\");");
    jsc.add(name + "FieldDescr.setGetMethod(\"get" + toUpperCaseFirstLetter(name) + "\");");
    ...
    jsc.add(name + "FM.setSetMethod(\"set" + toUpperCaseFirstLetter(name) + "\");");
    jsc.add(name + "FM.setGetMethod(\"get" + toUpperCaseFirstLetter(name) + "\");");
  ...
  /**
   * Creates the source code of the FieldInfo part with one-to-one relation appeding it to the
   * source code generated till now.
   * 
   * @param fInfo FieldInfo object
   * @param jsc JSourceCode created till now
   * @return JSourceCode created in this method
  ...
  private JSourceCode createOneToOneFieldInfoPart(final FieldInfo fInfo, final JSourceCode jsc) {
    ...
    // -- parent class descriptor
    jsc.add(name + "FieldDescr.setContainingClassDescriptor(this);");
    jsc.add(name + "FieldDescr.setClassDescriptor(new " + getLocalName(type) + "JDODescriptor());");
  ...
  private JSourceCode createOneToManyFieldInfoPart(final FieldInfo fInfo, final JSourceCode jsc) {
    ...
    jsc.add(
        name + "FieldJdoNature.setReadOnly(" + Boolean.toString(manyNature.isReadOnly()) + ");");
