...
public abstract class AbstractDigitsFacet extends AbstractRangeFacet {
  ...
  /**
   * If set to true only '0' is allowed for the fractionDigits facet. If set to false all positive
   * values are allowed for fractionDigits facet.
   */
  ...
  /**
   * Construct a new AbstractDigitsFacet optionally allowing the fractionDigits facet to be set to
   * all positive values.
   *
   * @param fractionDigitsZeroOnly If set to true only '0' is allowed for the fractionDigits facet.
   *        If set to false all positive values are allowed for fractionDigits facet.
   */
  ...
  protected final void codeDigitsFacet(final JSourceCode jsc, final String validatorName) {
    // -- totalDigits
    if (getTotalDigits() != -1) {
      jsc.add("{0}.setTotalDigits({1});", validatorName, Integer.toString(getTotalDigits()));
    }

    // -- fractionDigits
    if (!_fractionDigitsZeroOnly && (getFractionDigits() != -1)) {
      jsc.add("{0}.setFractionDigits({1});", validatorName, Integer.toString(getFractionDigits()));
    }
  }
