...
public class IdentityMemberAndAccessorFactory extends FieldMemberAndAccessorFactory {
   ...
   */
  private JMethod makeGetMethod(final FieldInfo fieldInfo, final String mname, final JType jType) {
    JMethod method = new JMethod("get" + mname, jType, "the value of field '" + mname + "'.");
   ...
   */
  private JMethod makeSetMethod(final FieldInfo fieldInfo, final String mname, final JType jType) {
  ...
  private JMethod makeGetReferenceIdMethod(final FieldInfo fieldInfo) {
    JMethod method = new JMethod("getReferenceId", SGTypes.STRING, "the reference ID");
    JSourceCode jsc = method.getSourceCode();
    jsc.add("return this.");
    jsc.append(fieldInfo.getName());
    jsc.append(";");
