...
/**
 * Strategy pattern for dealing with class name conflicts during source code generation. The various
 * implementations of this stragtegy will implement individual approaches for dealing with such a
 * naming collision.
 *
...
public interface ClassNameCRStrategy {
   ...
   */
  SGStateInfo dealWithClassNameConflict(final SGStateInfo state, final ClassInfo newClassInfo,
      final JClass conflict);

  /**
   * Implements a specific strategy for dealing with the fact that -- for a given file name -- an
   * artifact with the same name already exists.
   *
  ...
  /**
   * Injects the {@link SingleClassGenerator} instance that actually is calling a method of this
   * strategy.
   *
