...
public class CollectionMemberAndAccessorFactory extends FieldMemberAndAccessorFactory {
  ...
  public void generateInitializerCode(final FieldInfo fieldInfo, final JSourceCode sourceCode) {
    ...
    if (!StringUtils.isEmpty(fieldInfo.getDefaultValue())) {
      String buffer = new StringBuilder().append(fieldInfo.getName()).append(".add(")
   ...
   */
  protected void createAddMethod(final CollectionInfo fieldInfo, final JClass jClass) {
    ...
    final JParameter parameter =
        new JParameter(fieldInfo.getContentType().getJType(), fieldInfo.getContentName());
  ...
  /**
   * Returns true if extra collection methods should be generated. The extra collection methods are
   * methods which return an actual reference to the underlying collection as opposed to an
   * enumeration, iterator, or copy.
   *
   * @param fieldInfo the collectionInfo to translate
   ...
   * @param jClass the jClass to add the method to.
   */
  private void createGetAsReferenceMethod(final CollectionInfo fieldInfo, final JClass jClass) {
    JMethod method =
        new JMethod(fieldInfo.getReadMethodName() + fieldInfo.getReferenceMethodSuffix(),
            fieldInfo.getXSList().getJType(), "a reference to the Vector backing this class");
   ...
   * @param jClass the jClass to add the method to.
   */
  private void createAddAndRemoveMethods(final CollectionInfo fieldInfo, final JClass jClass) {
    // create add methods
   ...
   */
  private void createGetAndSetMethods(final CollectionInfo fieldInfo, final JClass jClass,
      final boolean useJava50, final AnnotationBuilder[] annotationBuilders) {
    // create get methods
   ...
   * @param jClass the jClass to add the method to.
   */
  private void createGetCountMethod(final CollectionInfo fieldInfo, final JClass jClass) {
  ...
  /**
   * Generate methods for iterating over the objects in the collection. For Java-1 collections, we
   * only generate an Enumerator. Implementations for other versions of Java should call this method
   * for backward compatbility and then add any additional new methods.
   *
   ...
   * @param useJava50 java version flag
   */
  protected void createIteratorMethod(final CollectionInfo fieldInfo, final JClass jClass,
      final boolean useJava50) {
   ...
   * @param jClass the jClass to add the method to.
   */
  private void createRemoveAllMethod(final CollectionInfo fieldInfo, final JClass jClass) {
  ...
  protected void createRemoveByIndexMethod(final CollectionInfo fieldInfo, final JClass jClass) {
    JMethod method = new JMethod("remove" + fieldInfo.getMethodSuffix() + "At",
        fieldInfo.getContentType().getJType(), "the element removed from the collection");
   ...
   * @param jClass the jClass to add the method to.
   */
  private void createRemoveObjectMethod(final CollectionInfo fieldInfo, final JClass jClass) {
   ...
   * @param useJava50 java version flag
   */
  private void createSetAsArrayMethod(final CollectionInfo fieldInfo, final JClass jClass,
      final boolean useJava50) {
    JMethod method = new JMethod("set" + fieldInfo.getMethodSuffix());
    final JParameter parameter =
        new JParameter(new JArrayType(fieldInfo.getContentType().getJType(), useJava50),
    ...
    sourceCode.append(
        fieldInfo.getContentType().createToJavaObjectCode(parameter.getName() + "[" + index + "]"));
    sourceCode.append(");");
    sourceCode.unindent();
    sourceCode.add("}");
   ...
   * @param jClass the jClass to add the method to.
   */
  private void createSetAsCopyMethod(final CollectionInfo fieldInfo, final JClass jClass) {
    JMethod method = new JMethod("set" + fieldInfo.getMethodSuffix());
    JParameter parameter =
        new JParameter(fieldInfo.getXSList().getJType(), fieldInfo.getContentName() + "List");
    method.addParameter(parameter);
   ...
   * @param useJava50 java version flag
   */
  private void createSetAsReferenceMethod(final CollectionInfo fieldInfo, final JClass jClass,
      final boolean useJava50) {
    JMethod method =
        new JMethod("set" + fieldInfo.getMethodSuffix() + fieldInfo.getReferenceSuffix());
    final JType collectionJType = new XMLInfoNature(fieldInfo).getSchemaType().getJType();
    JParameter parameter = new JParameter(collectionJType,
        fieldInfo.getParameterPrefix() + collectionJType.getLocalName());
    method.addParameter(parameter);
  ...
  protected void createSetByIndexMethod(final CollectionInfo fieldInfo, final JClass jClass) {
    ...
    sourceCode
        .append(fieldInfo.getContentType().createToJavaObjectCode(fieldInfo.getContentName()));
    sourceCode.append(");");
   ...
   * @param sourceCode the sourceCode to attach to
   */
  protected void addMaxSizeCheck(final CollectionInfo fieldInfo, final String methodName,
      final JSourceCode sourceCode) {
   ...
   * @param sourceCode the sourceCode to attach to
   */
  private void addIndexCheck(final CollectionInfo fieldInfo, final JSourceCode sourceCode,
