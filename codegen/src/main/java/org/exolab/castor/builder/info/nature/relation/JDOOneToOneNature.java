...
/**
 * A {@link JDOOneToOneNature} defines a one-to-one relation between two {@link FieldInfo}s.
 * 
...
public final class JDOOneToOneNature extends BaseNature {
   ...
   */
  public JDOOneToOneNature(final FieldInfo field) {
    super(field);
  }
  ...
  /**
   * Returns true if no update on the column can be performed, false otherwise. Default value is
   * false.
   * 
   ...
   * false.
   * 
   * @param dirty true if field should not be updated.
   */
  ...
  /**
   * Returns a List of {@String}s holding the columns of the foreign key. Keep in mind that by
   * contract of <code>addPrimaryKey(String foreignKey)</code> the order is not guaranteed.
   * 
   * @return the names of the foreign key's columns or null if no keys added before.
   */
  ...
  /**
   * Adds a column to the foreign key. By contract, the order of the key columns is not guaranteed
   * and depends on the returned List implementation the {@link BaseNature} is using.
   * 
