...
public final class EnumerationFactory extends BaseFactory {
  ...
  /**
   * Current (hence maximum) suffix for init methods, used to avoid the static initializer limits of
   * a JVM.
   */
   ...
   */
  void processEnumerationAsNewObject(final ExtendedBinding binding, final SimpleType simpleType,
    ...
    while (enumeration.hasMoreElements()) {
      ...
      if (addInitializerCode) {
        ...
        if (generateConstantDefinitions) {
          ...
          String init = new StringBuilder(32).append("new ").append(className).append('(')
              .append(count).append(", \"").append(escapeValue(value)).append("\")").toString();
          jsc.append(init);
        }
        jsc.append(");");
      }
  ...
  private void createJava5Enum(final SimpleType simpleType, final FactoryState state,
      final XMLBindingComponent component, final boolean useValuesAsName,
      final Enumeration<Facet> enumeration) {
    ...
    JField enumConstantsField =
        new JField(new JClass("java.util.Map<java.lang.String, " + jEnum.getLocalName() + ">"),
            "enumConstants");
    ...
    enumConstantsField
        .setInitString("new java.util.HashMap<java.lang.String, " + jEnum.getLocalName() + ">()");
    ...
    sourceCode.indent();
    sourceCode.indent();
    sourceCode.add(jEnum.getLocalName() + "." + enumConstantsField.getName() + ".put(c."
    ...
    while (enumeration.hasMoreElements()) {
      ...
      if (useValuesAsName) {
        enumConstant =
            new JEnumConstant(translateEnumValueToIdentifier(component.getEnumBinding(), facet),
  ...
  private void addValueMethod(JEnum jEnum) {
    JMethod valueMethod =
        new JMethod("value", new JClass("java.lang.String"), "the value of this constant");
  ...
  private JMethod createInitMethod(final JClass jClass) {
    ...
    JMethod mInit = new JMethod(initMethodName, SGTypes.createHashtable(getConfig().useJava50()),
   ...
   * @param state our current state
   */
  void processEnumerationAsBaseType(final ExtendedBinding binding, final SimpleType simpleType,
      final FactoryState state) {
   ...
   * is through a couple of simple rules:
   * <ul>
   * <li>if the value parses as a non-negative int, the string 'VALUE_' is prepended to it</li>
   * <li>if the value parses as a negative int, the string 'VALUE_NEG_' is prepended to it</li>
   * <li>the value is uppercased</li>
   * <li>the characters <code>[](){}<>'`"</code> are removed</li>
   * <li>the characters <code>|\/?~!@#$%^&*-+=:;.,</code> and any whitespace are replaced with
   * <code>_</code></li>
   * </ul>
   * 
  ...
  private String translateEnumValueToIdentifier(final EnumBindingType enumBinding,
    ...
    try {
      ...
      if (enumBinding != null) {
        customMemberName = getCustomMemberName(enumBinding, String.valueOf(intVal));
      }
   ...
   */
  private String getCustomMemberName(final EnumBindingType enumBinding, final String enumValue) {
