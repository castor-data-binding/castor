...
public final class XMLFieldHandlerFactory {
   ...
   */
  public void createXMLFieldHandler(final FieldInfo member, final XSType xsType,
      final String localClassName, final JSourceCode jsc, final boolean forGeneralizedHandler) {
   ...
   * @param isAttribute Whether to create a setValue() method for an attribute.
   * @param isContent Whether to create a setValue() method for XML content.
   */
  private void createSetValueMethod(final FieldInfo member, final XSType xsType,
      final String localClassName, final JSourceCode jsc, final boolean any,
      final boolean isAttribute, final boolean isContent) {
    if (_config.useJava50()) {
      jsc.add("@Override");
    }
    ...
    if (xsType.isPrimitive() && !_config.usePrimitiveWrapper()) {
      if ((!xmlNature.isRequired()) && (!xsType.isEnumerated()) && (!xmlNature.isMultivalued())) {
   ...
   * @param localClassName Name of the object instance as used locally
   */
  private void createResetMethod(final FieldInfo member, final String localClassName,
   ...
   */
  private void createNewInstanceMethod(final FieldInfo member, final XSType xsType,
      final JSourceCode jsc, final boolean forGeneralizedHandler, final boolean any,
    ...
    if (any || forGeneralizedHandler || isEnumerated || xsType.isPrimitive()
        || xsType.getJType().isArray() || (xsType.getType() == XSType.STRING_TYPE) || isAbstract) {
      jsc.append("null;");
    } else {
