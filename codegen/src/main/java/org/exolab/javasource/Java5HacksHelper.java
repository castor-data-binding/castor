...
/**
 * Makes an effort to identify where Override annotations belong. We are limited in what we look at
 * and we do not use reflection to identify if a class truly extends the appropriate class. We only
 * check the class of the arguments. This class may not be generically useful, but is useful with
 * the code generated by Castor. <br/>
 * Our one public method, {@link #addOverrideAnnotations(JMethodSignature)}, should only be called
 * when you already know that you are generating Java-5 style code.
 * 
...
public final class Java5HacksHelper {
  ...
  private static class MethodSpec {
    ...
    public MethodSpec(final String methodName, final int paramCount, final String param1ClassName,
        final String param2ClassName) {
