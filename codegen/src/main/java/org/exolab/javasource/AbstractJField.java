...
/**
 * A (abstract) base class which holds information about fields. Modeled closely after the Java
 * Reflection API. This class is part of package which is used to create source code in memory.
 *
...
public class AbstractJField extends JAnnotatedElementHelper implements JMember {
   ...
   *
   * @return The JavaDoc comment describing this member, or null if no comment has been set.
   */
   ...
   *
   * @return The initialization String for this JField, or null if no initialization String was
   *         specified.
   */
  ...
  /**
   * Sets the initialization string for this JField. This allows some flexibility in declaring
   * default values.
   *
  ...
  public void setName(final String name) {
    if (!JNaming.isValidJavaIdentifier(name)) {
      ...
      if (JNaming.isKeyword(name)) {
        err += "a reserved word and may not be used as " + " a field name.";
      } else {
  ...
  public String toString() {
    return new StringBuilder().append(_modifiers).append(' ').append(_type).append(' ')
