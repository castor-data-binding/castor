...
/**
 * The XMLProperties test has one goal - check if the proper defaults are in place.
 * 
...
public class XMLPropertiesTest extends TestCase {
  ...
  public void setUp() {
    ...
    Assert.assertNotNull("Properties must exist after call to newInstance()", _properties);
    Assert.assertNotNull("Application class loader must not be null",
        _properties.getApplicationClassLoader());
    Assert.assertNotNull("Domain class loader must not be null",
  ...
  public void testNewInstanceClassLoaderClassLoader() {
    ...
    Assert.assertNotNull("Properties must exist after call to newInstance()", properties);
  ...
  public void testGetBooleanStringBoolean() {
    ...
    Assert.assertTrue("A not existing property with default true", notExistingPropertyWithDefault);
  }
  ...
  public void testDefaults() {
    Boolean loadPackageMapping = _properties.getBoolean(XMLProperties.LOAD_PACKAGE_MAPPING);
    Assert.assertEquals("load package mapping is expected to be set to: true", Boolean.TRUE,
        loadPackageMapping);

    String serialzierFactory = _properties.getString(XMLProperties.SERIALIZER_FACTORY);
    Assert.assertEquals("check serializer factory default",
        "org.exolab.castor.xml.XercesJDK5XMLSerializerFactory", serialzierFactory);
