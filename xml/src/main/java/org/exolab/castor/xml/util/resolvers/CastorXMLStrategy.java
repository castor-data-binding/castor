 ...
 * before this refactoring step. Meaning that:<br/>
 * It uses multiple steps to find a class descriptor for a class.<br/>
 * It uses a cache of class descriptors<br/>
 * A class that couldn't be resolved once is marked as unresolvable and will not be resolved again -
 * even on a second call.<br/>
 * 
...
public class CastorXMLStrategy implements ResolverStrategy {
  ...
  /**
   * CastorXMLStrategy requires a configuration to be set. Within the constructor the commands
   * building the strategy are instantiated, a command configuration is created and the descriptor
   * cache.
   */
   ...
   * <ul>
   * <li>The resolver results are checked in the beginning and after each command executed
   ...
   * <li>The class file of the corresponding descriptor class (which is className + "Descriptor")
   * </ul>
   * <br/>
   * If any of these resources yield an XMLClassDescriptor it is added to the internal cache and
   * returned as result.
   * 
   ...
   * @return An <code>XMLClassDescriptor</code> for the given class or <code>null</code> if no
