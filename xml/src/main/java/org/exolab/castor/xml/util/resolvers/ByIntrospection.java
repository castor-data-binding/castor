...
public class ByIntrospection extends AbstractResolverClassCommand {
   ...
   * on the <code>Introspector</code> set with <code>setIntrospector</code>. If a descriptor is
   * successfully created it will be added to the DescriptorCache. <br>
   * <b>NOTE</b>: If this XMLClassDescriptorResolver is NOT configured to use introspection this
   * method will NOT create an descriptor.<br>
   * <br>
   * {@inheritDoc}
   */
  protected Map internalResolve(final String className, final ClassLoader classLoader,
    ...
    if (clazz != null) {
      try {
        ...
        String message =
            "Failed to generate class descriptor for: " + clazz + " with exception: " + e;
        LOG.warn(message);
