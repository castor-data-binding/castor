...
/**
 * The abstract resolver command provides the argument checking, writes a debug message and reads
 * the class loader from the properties... All specific code is found in the extended classes.
 * 
...
public abstract class AbstractResolverPackageCommand implements ResolverPackageCommand {
  ...
  public final Map<String, ClassDescriptor> resolve(final String packageName, final Map properties)
    ...
    if (LOG.isDebugEnabled()) {
      LOG.debug(
          "Now in resolve method: " + this.getClass().getName() + " resolving: " + packageName);
    }

    ClassLoader classLoader = (ClassLoader) properties.get(ResolverStrategy.PROPERTY_CLASS_LOADER);
    if (classLoader == null) {
  ...
  protected abstract Map<String, ClassDescriptor> internalResolve(String packageName,
      ClassLoader classLoader, Map props) throws ResolverException;
}
