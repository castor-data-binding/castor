...
class IDResolverImpl implements IDResolver {
  ...
  /**
   * A custom (user-injected) IDResolver instance to be used for IDREF resolution.
   */
   ...
   * @param object Object being identified by ID
   * @param isValidating True if validation is enabled.
  ...
  void bind(final String id, final Object object, final boolean isValidating)
    ...
    if (isValidating && _idReferences.containsKey(id)) {
      if (!(id.equals("org.exolab.castor.mapping.MapItem") || id.equals("HIGH-LOW"))) {
