...
public class ByCDR extends AbstractResolverPackageCommand {
   ...
   * this cache. <br>
   * If a descriptor is listed in the CDR file for the given package but could not be loaded (e.g.
   * because the reference class file is not available) the descriptor is ignored but no exception
   * is thrown. <br>
   * If a CDR file is not available for the given package this method will not load any descriptors
   * and not throw any exceptions. <br>
   * Further calls to this method with the same package name will not be processed. <br>
   * {@inheritDoc}
   */
  protected Map<String, ClassDescriptor> internalResolve(final String packageName,
      final ClassLoader classLoader, final Map properties) throws ResolverException {
    ...
    URL url = classLoader.getResource(
        ResolveHelpers.getQualifiedFileName(XMLConstants.PKG_CDR_LIST_FILE, packageName));
    if (url == null) {
      return results;
    }
      ...
      for (Object clazz : cdrList.keySet()) {
        ...
        try {
          ...
          if (descriptorClass != null) {
            ...
            LOG.debug("Loading of descriptor class: " + descriptorClassName + " for class: "
                + clazzName + " has failed - continue without");
          }
        } catch (Exception e) {
          if (LOG.isDebugEnabled()) {
            LOG.debug(
                "Ignored problem at loading of: " + descriptorClassName + " with exception: " + e);
          }
        }
      }
