...
/**
 * Implements a test case that tests code written by the XML source generator. This class uses a
 * user-provided test class to test the generated source.
 * <p>
 * Each user-provided test is allowed to return a Boolean, either a primitive or a java.lang.Boolean
 * -- it does not matter. If the user-provided test returns a Boolean and it is false, then the test
 * is considered to have failed. If the user-provided test throws <i>or returns</i> a Throwable, it
 * is considered to have failed. If the user-provided test returns <b>anything else</b> (including
 * void) then the test is considered to have passed.
 * <p>
 * Note: Returning Throwable is a little bit cleaner than throwing an Exception, but either is
 * acceptable as a sign of test failure. This is because when a Throwable is returned, if
 * -printStack is in effect, then the CORRECT stack trace can be displayed and not a stack dump from
 * the refective invocation.
 * <p>
 * There is no requirement that the user-provided test implement any interface, nor any requirement
 * that the user-provided test return anything at all. However, a test that returns "void" and that
 * never throws an Exception is not a very useful test as it can never fail.
 *
...
class TestWithCustomTest extends TestCase {
  ...
  public void runTest() {
    ...
    for (Iterator i = returnValues.iterator(); i.hasNext(); count++) {
      ...
      if (o instanceof Boolean && !((Boolean) o).booleanValue() && !_failureExpected) {
        ...
        testFailed = true;
        System.out
            .println("Custom test #" + count + " was expected to succeed, but returned false");
      }
      ...
      if (o instanceof Throwable && !_failureExpected) {
        testFailed = true;
        System.out
            .println("Custom test #" + count + " was expected to succeed, but returned Throwable");
   ...
   */
  protected Object getTestObject(final String testClassName)
