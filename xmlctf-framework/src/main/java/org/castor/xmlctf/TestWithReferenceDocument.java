...
/**
 * Implements a test case that tests code written by the XML source generator. This class uses the
 * generated source to read and write an XML document, comparing the XML document written against
 * the reference document that was originally read in.
 * <p>
...
class TestWithReferenceDocument extends TestCase {
  ...
  /**
   * Blank constructor for this test case. This contructor is not useful, since no delegate test
   * case is provided.
   * 
  ...
  /**
   * Provides setup for our delegated test case, depending on the type of test case we are
   * delegating for.
   * 
  ...
  /**
   * Provides tear down for our delegated test case, depending on the type of test case we are
   * delegating for.
   * 
  ...
  public void runTest() throws Exception { // FIXME - temporarily throws Exception
    ...
    if (ref == null) {
      throw new Exception(
          "There is no valid input file or hardcoded object in '" + _delegate._name + "'");
    }
    ...
    if (_goldFileName != null && _goldFileName.length() > 0) {
      int result = CTFUtils.compare(_delegate._outputRootFile + "/" + _goldFileName,
          marshal_output.getAbsolutePath());
      verbose("----> Compare marshaled document to gold file '" + _goldFileName + "': "
          + ((result == 0) ? "OK" : "### Failed ### "));

      final boolean expectedToFail =
          _failure != null && _failure.getContent() && _failure.getFailureStep() != null
      ...
      if (_failure == null || !_failure.getContent()) {
        ...
        assertTrue(
            "The Marshaled object was expected to differ from the" + " gold file, but did not",
            result > 0);
      }
    }
  ...
  /**
   * Returns a reference document as generated by unmarshaling the input document provided, if one
   * was provided.
   * 
  ...
  /**
   * Returns a reference document as generated by the hard-coded ObjectModelBuilder.
   *
   * @return a reference document as generated by the hard-coded ObjectModelBuilder
   * @throws Exception if anything goes wrong executing creating the ObjectModelBuilder or using it
   *         to create the reference document.
   */
  ...
  /**
   * Make sure the ObjectModelBuilder object and the unmarshaled input document created the same
   * object.
   * 
   ...
   */
  private void compareReferenceObjects(Object refUnmarshal, Object refGenerated)
      throws IOException {
    // the object model must override the equals method.
    boolean result = refGenerated.equals(refUnmarshal);
    verbose("----> Compare unmarshaled document to reference object: "
        + ((result) ? "OK" : "### Failed ### "));
    if (result == false) {
      verbose("Make sure the reference object model overrides Object#equals");
    }
    ...
    if (result == false && refGenerated instanceof CastorTestable) {
      ...
      FileWriter writer =
          new FileWriter(new File(_delegate._outputRootFile, _outputName + "-refModel.dump"));
      writer.write(((CastorTestable) refGenerated).dumpFields());
      writer.close();

      writer =
          new FileWriter(new File(_delegate._outputRootFile, _outputName + "-refUnmarshal.dump"));
      writer.write(((CastorTestable) refUnmarshal).dumpFields());
      writer.close();
    }

    assertTrue("The unmarshaled reference object differs from the hardcoded reference object.",
        result);
  }

  /**
   * Tests that the listener saw what it was supposed to see. If we have a listener configured,
   * marshal it to disk (now that it listened to the previous marshal) and compare it to the
   * listener gold file, if any.
   * <p>
   * We have to unregister the listener before we do anything. When we marshal the MarshalListener,
   * for every item marshaled the listener may grow and may have more to marshal, and thus we may
   * end up in an endless loop. For example, a simple implementation of MarshalListener could log
   * each pre/post marshal invocation on a Vector to allow for later comparisons. But this means
   * that the object *being marshaled* keeps getting data added to it during the marshaling!
   *
  ...
  private void compareListenerToItsGoldFile() throws Exception {
    ...
    int result = CTFUtils.compare(_delegate._outputRootFile + "/" + _delegate._listenerGoldFile,
        outputFile.getAbsolutePath());
    verbose("----> Compare marshaled document to gold file '" + _delegate._listenerGoldFile + "': "
        + ((result == 0) ? "OK" : "### Failed ### "));
