...
/**
 * Compiles a directory tree, recursively. This class is built to use the Oracle
 * tool JavaCompiler contained in tools.jar. A IllegalStateException will be
 * thrown if tools.jar is not on the classpath at construction of the class and
...
public class OracleJavaCompiler implements Compiler {
	...
	private static final float JAVA_VERSION = Float.parseFloat(System
	...
	public OracleJavaCompiler(File baseDirectory) {
		if (baseDirectory == null) {
			throw new IllegalArgumentException(
					"'baseDirectory' must not be null.");
		}
	...
	/**
	 * Compiles the content of a directory. Throws a
	 * <code>CompilationException</code> if the build fails.
	 */
	public void compileDirectory() {
		...
		if (filesList.size() > 0) {
			...
			Iterable<String> compileOptions = getCompileArguments(
					_baseDirectory, _outputDirectory);
			// Iterable<String> compileOptions = Arrays.asList("-g", "-d",
			// "-cp", CLASSPATH);
			Writer logger = new StringWriter();
			JavaCompiler.CompilationTask task = compiler.getTask(logger,
					fileManager, null, compileOptions, null, compilationUnits);
			...
			if (ok == false) {
				throw new IllegalStateException(
						"Failed to call compile method.");
			}

		} else {
			throw new CompilationException("No files to compile: "
					+ _baseDirectory);
		}
	}
	 ...
	 * 
	 * @param srcDir
	 *            The source directory for compilation
	 * @param destDir
	 *            The destination directory for compilation
	 * @return a list of arguments for the compiler.
	 */
	private List<String> getCompileArguments(final File srcDir,
			final File destDir) {
		List<String> args = new ArrayList<String>();

		args.add("-g");
		if (JAVA_VERSION == 1.5F || JAVA_VERSION == 1.6F
				|| JAVA_VERSION == 1.7F) {
		...
		args.add("-classpath");
		String classPathOverriden = System
				.getProperty("xmlctf.classpath.override");
	...
	/**
	 * Recursively searches the provided directory, returning a list of all Java
	 * files found.
	 * 
	 * @param sourceDirectory
	 *            A directory to search for Java files.
		...
		for (int i = 0; i < entries.length; i++) {
			...
			if (entry.getName().endsWith(".java")) {
				files.add(entry);
					...
					&& !IGNORE_DIRS.contains(entry.getName())) {
