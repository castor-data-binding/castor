...
/**
 * Compiles a directory tree, recursively. This class is built to use the Sun Javac compiler
    ...
    if (filesList.size() > 0) {
      ...
      switch (status) {
        ...
        case COMPILATION_ERROR:
          throw new CompilationException("Compile status: ERROR");
        case COMPILATION_CMDERR:
          throw new CompilationException("Compile status: CMDERR");
        case COMPILATION_SYSERR:
          throw new CompilationException("Compile status: SYSERR");
        case COMPILATION_ABNORMAL:
          throw new CompilationException("Compile status: ABNORMAL");
        default:
          throw new CompilationException("Compile status: Unknown");
      }
    } else {
