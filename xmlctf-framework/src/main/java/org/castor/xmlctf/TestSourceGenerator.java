...
/**
 * This class encapsulate all the logic needed to run the source generator by itself and then
 * compile the file that have been generated. This class does not do anything additional. It only
 * runs the source generator and ensures that the generated source will compile without error.
 *
...
public class TestSourceGenerator extends XMLTestCase {
  ...
  public TestSourceGenerator(final CastorTestCase test, final UnitTestCase unit,
  ...
  /**
   * Runs our source generation test. Creates, configures, and executes the source generator on each
   * schema we have to test. Compiles the generated code. Loads classes via the appropriate class
   * loader.
   */
  public void runTest() {
    if (_skip) {
   ...
   * @throws IOException if any IOException occurs preparing the source generator.
   */
  private SourceGenerator createSourceGenerator() throws IOException {
    ...
    if (_propertyFileName != null) {
      if (!(new File(_test.getTestFile(), _propertyFileName)).exists()) {
        fail("Test properties file '" + _propertyFileName
            + "' does not exist; check TestDescriptor.xml");
      }
