...
public class TestUpdateOneToMany_NullAccepted extends CPATestCase {
	...
	// load objects again and check
	_db.begin();
	Book db_book2 = _db.load(Book.class, 12);
	_db.commit();
	
	clearTables();
	_db.close();
	
	assertEquals(db_book2.getAuthor(), null);
    }
    
    public void testUpdate_setAuthor() throws Exception {
	// cascading must be independent of autostore
	_db.setAutoStore(false);
	...
	assertEquals(db_author.getName(), "Jack");	
	assertEquals(db_book2.getAuthor().getId(), 11);
    }
