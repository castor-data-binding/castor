...
/**
 * An implementation of <b>maxInclusive</b> constraining facet, defined in section "<a href="
 * http://www.w3.org/TR/xmlschema-2/#rf-maxInclusive">4.3.7 maxInclusive</a>" of "<a href="
 * http://www.w3.org/TR/xmlschema-2/">XML Schema Part 2: Datatypes Second Edition</a>" document.
 *
 * <p>
 * [Definition:] <b>maxInclusive</b> is the <i>inclusive upper bound</i> of the <i>value space</i>
 * for a datatype with the <i>ordered</i> property. The value of <b>maxInclusive</b> <i>must</i> be
 * in the <i>value space</i> of the <i>base type</i>.
 *
...
public final class MaxInclusive extends Facet {
   ...
   *
   * <p>
   * <i>maxInclusive</i> can override the following facets of the base data type:
   *
   * <ul>
   ...
   * @return <code>true</code>, if the current facet overrides <code>baseFacet</code>;
  ...
  public boolean overridesBase(final Facet baseFacet) {
    ...
    return otherName.equals(Facet.MAX_EXCLUSIVE) || otherName.equals(Facet.MAX_INCLUSIVE);
  }

  /**
   * Validation is performed according to section "<a href="
   * http://www.w3.org/TR/xmlschema-2/#maxInclusive-coss">4.3.7.4 Constraints on maxInclusive Schema
   * Components</a>" of "<a href="http://www.w3.org/TR/xmlschema-2/">XML Schema Part 2: Datatypes
   * Second Edition</a>" document.
   *
   * @param localFacets local facets of the data type
   * @param baseFacets merged facets of the base data type
   * @throws SchemaException when the current facet does not satisfy schema component validation
   *         constraints
   */
  public void checkConstraints(final Enumeration localFacets, final Enumeration baseFacets)
    ...
    while (localFacets.hasMoreElements()) {
      ...
      if (otherName.equals(Facet.MIN_INCLUSIVE) && getOwningType().isNumericType()
          && other.toBigDecimal().compareTo(this.toBigDecimal()) > 0) {
        // Schema Component Constraint: minInclusive <= maxInclusive
        throw new SchemaException(
            "It is an error for the value specified " + "for minInclusive to be greater than "
                + "the value specified for maxInclusive " + "for the same datatype.");
      }
    }
    if (baseFacets != null) {
      while (baseFacets.hasMoreElements()) {
        ...
        if (otherName.equals(Facet.MAX_INCLUSIVE) && getOwningType().isNumericType()
            && this.toBigDecimal().compareTo(other.toBigDecimal()) > 0) {
          // [1]
          throw new SchemaException("It is an error if the following condition is true: "
        ...
        } else if (otherName.equals(Facet.MAX_EXCLUSIVE) && getOwningType().isNumericType()
            && this.toBigDecimal().compareTo(other.toBigDecimal()) >= 0) {
          // [2]
          throw new SchemaException("It is an error if the following condition is true: "
        ...
        } else if (otherName.equals(Facet.MIN_INCLUSIVE) && getOwningType().isNumericType()
            && this.toBigDecimal().compareTo(other.toBigDecimal()) < 0) {
          // [3]
          throw new SchemaException("It is an error if the following condition is true: "
        ...
        } else if (otherName.equals(Facet.MIN_EXCLUSIVE) && getOwningType().isNumericType()
            && this.toBigDecimal().compareTo(other.toBigDecimal()) <= 0) {
          // [4]
          throw new SchemaException("It is an error if the following condition is true: "
