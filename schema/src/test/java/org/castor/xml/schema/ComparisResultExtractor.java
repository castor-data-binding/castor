...
public class ComparisResultExtractor {
  ...
  public static final ComparisonResult doTest(Schema schema, URL expected) throws Exception {
    ...
    XMLDiff diff =
        new XMLDiff(targetedOutput.getAbsolutePath(), new File(expected.toURI()).getAbsolutePath());
    int result = diff.compare();
    ComparisonResult testResult =
        result == 0 ? ComparisonResult.IDENTICAL : ComparisonResult.DIFFERENCE;
