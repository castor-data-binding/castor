...
/**
 * Extension of {@link AnnotationProcessingService} to handle target-aware {@link Annotation}s.
 * 
...
public interface TargetAwareAnnotationProcessingService extends AnnotationProcessingService {
  ...
  /**
   * Calls {@link #processAnnotation(BaseNature, Annotation)} for each given Annotation.
   * 
   * @param info the {@link BaseNature} (and so its {@link PropertyHolder}) that should be filled
   *        with the information read
   * @param annotations the annotations to process
   * @param target the target ({@link AnnotatedElement}) of the given annotation
   * @return Annotation[] filled with unprocessed annotations
   * @throws AnnotationTargetException if an annotation is used in a context that is not valid.
   * @see #processAnnotation(BaseNature, Annotation)
   */
  public abstract <I extends BaseNature> Annotation[] processAnnotations(I info,
      final Annotation[] annotations, final AnnotatedElement target)
      throws AnnotationTargetException;

  /**
   * The processing action of this service. If an annotation is given which is not supported by this
   ...
   * 
   * @param annotationProcessor the {@link TargetAwareAnnotationProcessor} to add to this service.
   */
  public void addAnnotationProcessor(final TargetAwareAnnotationProcessor annotationProcessor);

  /**
   * Returns the set of {@link TargetAwareAnnotationProcessor}s registered with this service.
   * 
   * @return A set of {@link TargetAwareAnnotationProcessor}s registered with this service.
   */
  public Set<TargetAwareAnnotationProcessor> getTargetAwareAnnotationProcessors();

  /**
   * Returns the set of {@link AnnotationProcessor}s and {@link TargetAwareAnnotationProcessor}s
