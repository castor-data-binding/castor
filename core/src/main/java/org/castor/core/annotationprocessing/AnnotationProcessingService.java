...
/**
 * An AnnotationProcessingService handles multiple {@link AnnotationProcessor}s and uses them to
 * process one or more {@link Annotation}s.
 * 
...
public interface AnnotationProcessingService {
   ...
   * {@link AnnotationProcessor#processAnnotation(BaseNature, Annotation)} is returned.
   * 
   * @param info the {@link BaseNature} (and so its {@link PropertyHolder}) that should be filled
   *        with the information read
   ...
   * 
   * @param annotationProcessor the {@link AnnotationProcessor} to add to this service.
   */
  public void addAnnotationProcessor(final AnnotationProcessor annotationProcessor);

  /**
   * Returns the set of {@link AnnotationProcessor}s registered with this service.
   * 
