...
/**
 * Adds property handle methods and a constructor including a health check. See constructor comments
 * for more details.
 * 
 * @author Lukas Lang
 * @since 1.2.1
...
public abstract class BaseNature implements Nature {
  ...
  /**
   * Constructor taking a {@link PropertyHolder}. Must be called from subclasses. Does a health
   * check on the given PropertyHolder, whether the extending Nature exists.
   * 
   * @param holder a PropertyHolder.
   */
  ...
  private String addPrefix(final String key) {
    return new StringBuilder().append(getId()).append(key).toString();
  }

  /**
   * Returns boolean value of the property or false if property value is null. Make sure, not to
   * request a property, which does not have a boolean value!
   * 
   * @param propertyName name of the property.
   * @return false if null or false.
   */
  protected final boolean getBooleanPropertyDefaultFalse(final String propertyName) {
  ...
  /**
   * Returns value of the property as a List. If the property was not set before, a new List will be
   * returned. Make sure, not to request a property, which is not a List!
   * 
   * @param propertyName name of the property.
   * @return A List.
   */
