...
public abstract class AbstractProperties {
  ...
  /**
   * Name of the system property that can be used to specify the location of user properties.
   */
  ...
  /**
   * {@link ClassLoader} to be used for all classes of Castor and its required libraries.
   */
  private final ClassLoader _applicationClassLoader;

  /**
   * {@link ClassLoader} to be used for all domain objects that are marshalled/unmarshalled or
   ...
   * to load the concrete properties class. No parent properties will be set.
   */
  ...
  /**
   * Get classloader to be used for all domain objects that are marshalled/unmarshalled or loaded
   * from the database.
   * 
  ...
  /**
   * Load common user properties from classpath root and current working directory. <br/>
   * First it loads default properties contained in Castor JAR. This gets overwritten by properties
   * found on Java library directory. If no properties could be found until that point a
   * PropertiesException will be thrown. At last overriding properties are loaded from root of
   * classpath or, if that could not be found, from local working directory.
   * 
   * @param filename Name of the properties file.
   */
  ...
  /**
   * Put given value associated with given key into the properties map of this properties. If the
   * properties previously associated the key to another value the previous value will be returned.
   * If a mapping for the key previously exist in the parent properties only, the method returns
   * <code>null</code> and not the value of the parent. This allows to distingush if the mapping
   * existed in this properties or one of its parents. <br/>
   * Putting a value in this properties does not change the value of its parent but the parents
   * value isn't visible any more as it gets overwritten by this properties one. While this allows
   * to redefine the value of a property it isn't allowed to undefine it. Therefore a
   * <code>NullPointerException</code> will be thrown if the given value is <code>null</code>.
   * 
   ...
   * previously associated with the key in this properties will be returned. If a mapping for the
   * key existed in the parent properties only, the method returns <code>null</code> and not the
   * value of the parent. This allows to distingush if the mapping existed in this properties or one
   * of its parents. <br/>
   * Removing the value from this properties does not mean that consecutive gets return
   * <code>null</code> as one of the parents may still contain a mapping for the key that was hidden
   * by the mapping in this properties.
   * 
   ...
   * equal, ignore case, to 'true' or 'false', the respective boolean value will be returned. If the
   * property is not found, <code>null</code> will be returned. For all other types and string
   * values a PropertiesException will be thrown. This behaviour is intended for those usecases that
   * need distinguish between values that are missconfigured or not specified at all.
   *
   ...
   * <br/>
   * If the key maps to a boolean value, it will be returned as is. For string values that are
   * equal, ignore case, to 'true' or 'false', the respective boolean value will be returned. In all
   * other cases the given default value will be returned.
   *
   * @param key Property key.
   ...
   * in this property map, the parent property map, and its parents, recursively, are then checked.
   * <br/>
   * If the key maps to a integer value, it will be returned as is. For string values that can be
   * interpreted as signed decimal integer, the respective integer value will be returned. If the
   * property is not found, <code>null</code> will be returned. For all other types and string
   * values a PropertiesException will be thrown. This behaviour is intended for those usecases that
   * need distinguish between values that are missconfigured or not specified at all.
   *
   ...
   * <br/>
   * If the key maps to a integer value, it will be returned as is. For string values that can be
   * interpreted as signed decimal integer, the respective integer value will be returned. In all
   * other cases the given default value will be returned.
   *
   * @param key Property key.
   * @param defaultValue Default value.
   ...
   * <code>null</code> will be returned. For all other types a PropertiesException will be thrown.
   *
   * @param key Property key.
   ...
   * in this property map, the parent property map, and its parents, recursively, are then checked.
   * <br/>
   * If the key maps to a string array, it will be returned as is. A simple string will be converted
   * into a string array by splitting it into substrings at every occurence of ',' character. If the
   * property is not found, <code>null</code> will be returned. For all other types a
   * PropertiesException will be thrown.
   *
   * @param key Property key.
   ...
   * in this property map, the parent property map, and its parents, recursively, are then checked.
   * <br/>
   * If the key maps to a class, it will be returned as is. A simple string will be interpreted as
   * class name of which the class will be loaded with the given class loader. If the property is
   * not found, <code>null</code> will be returned. For all other types and if loading of the class
   * fails a PropertiesException will be thrown.
   *
   * @param key Property key.
   ...
   * in this property map, the parent property map, and its parents, recursively, are then checked.
   * <br/>
   * If the key maps to a class array, it will be returned as is. A simple string will be splitted
   * it into substrings at every occurence of ',' character. Each of these substrings will
   * interpreted as class name of which the class will be loaded with the given class loader. If the
   * property is not found, <code>null</code> will be returned. For all other types and if loading
   * of one of the classes fails a PropertiesException will be thrown.
   *
   * @param key Property key.
   * @param loader Class loader to load classes with.
   ...
   * in this property map, the parent property map, and its parents, recursively, are then checked.
   * <br/>
   * If the key maps to a object array, it will be returned as is. A simple string will be splitted
   * it into substrings at every occurence of ',' character. Each of these substrings will
   * interpreted as class name of which the class will be loaded with the given class loader and
   * instantiated using its default constructor. If the property is not found, <code>null</code>
   * will be returned. For all other types and if loading or instantiation of one of the classes
   * fails a PropertiesException will be thrown.
   *
   * @param key Property key.
   * @param loader Class loader to load classes with.
