...
/**
 * Class encodes the bytes written to the OutPutStream to a Base64 encoded string. The encoded
 * string can be retrieved by as a whole by the toString() method or splited into lines of 72
 * characters by the toStringArray() method.
 *
...
public final class Base64Encoder {
  ...
  private static final char[] MAP = {'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', // 00-07
  ...
  /**
   * Reset Base64Encoder to its initial state. Take care using this method as it throws all
   * previously written bytes away.
   */
  public void reset() {
    _buffer = 0;
    _octets = 0;
  ...
  /**
   * Translate all bytes of given array by appending each to octet buffer. If buffer contains 3
   * octets its content will be encoded to 4 sextet byte values which are converted to a base64
