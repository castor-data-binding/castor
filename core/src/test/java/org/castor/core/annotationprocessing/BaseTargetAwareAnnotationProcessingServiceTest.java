...
public class BaseTargetAwareAnnotationProcessingServiceTest extends TestCase {
  ...
  /**
   * This is a {@link TargetAwareAnnotationProcessor} that actually does nothing, except telling
   * that it processed the given Annotation if it was of Type {@link SupportedAnnotation}. It's just
   * a dummy.
   */
  class SupportedClassAnnotationProcessor implements TargetAwareAnnotationProcessor {
    ...
    public <I extends BaseNature, A extends Annotation> boolean processAnnotation(I info,
        A annotation, AnnotatedElement target) {
  ...
  /**
   * A simple empty class, that just has 2 Annotations - for one ( {@link SupportedAnnotation}) we
   * have a dummy processor, for the other one ( {@link UnsupportedAnnotation}) we don't have a
   * processor.
   * 
   */
  @SupportedAnnotation
  ...
  /**
   * A dummy property holder needed for use with {@link BaseNature}s. In real usage within Castor,
   * this would be a ClassInfo or FieldInfo.
   * 
   */
  ...
  protected void setUp() throws Exception {
    assertTrue(AnnotationHolder.class.isAnnotationPresent(SupportedAnnotation.class));
    assertTrue(AnnotationHolder.class.isAnnotationPresent(UnsupportedAnnotation.class));
    assertTrue(AnnotationHolder.class.getMethod("annotatedMethod", new Class<?>[0])
  ...
  public void testAddGetAnnotationProcessor() {
    BaseTargetAwareAnnotationProcessingService baps =
        new BaseTargetAwareAnnotationProcessingService();
  ...
  public void testProcessAnnotationsTargetAware()
      throws SecurityException, NoSuchMethodException, AnnotationTargetException {
    BaseTargetAwareAnnotationProcessingService baps =
        new BaseTargetAwareAnnotationProcessingService();
    ...
    ArrayList<Class<? extends Annotation>> unprocessedList =
  ...
  public void testProcessAnnotation() {
    ...
    Annotation unsupportedAnnotation =
        AnnotationHolder.class.getAnnotation(UnsupportedAnnotation.class);
    Annotation supportedAnnotation =
        AnnotationHolder.class.getAnnotation(SupportedAnnotation.class);
    assertNotNull(unsupportedAnnotation);
    assertNotNull(supportedAnnotation);

    /*
     * we know, that we don't have processor for @UnsupportedAnnotation, so processing it will
     * return false.
     */
