...
/**
 * Runs various tests on the {@link BaseNature}. Due to visibility restrictions, an inner class
 * extending the {@link BaseNature} is defined.
 * 
 * @author Lukas Lang
 * @since 1.2.1
 * 
...
public final class BaseNatureTest extends TestCase {
  ...
  /**
   * Set properties via two Natures on the same {@link ClassInfo} using the same Key. Values must
  ...
  /**
   * Error Case. Should throw a NullPointerException if null param in constructor is given.
   */
  ...
  public void testId() {
    ...
    ci.addNature(BaseNatureWrapper.class.getName());
    BaseNatureWrapper bnw = new BaseNatureWrapper(ci);
    assertEquals("org.castor.core.nature.BaseNatureTest$BaseNatureWrapper", bnw.getId());
  ...
  /**
   * Checks, if default false strategy for properties not set is well implemented.
   */
  ...
  /**
   * Checks, if default false strategy for properties not set is well implemented. Property was set
   * before.
   */
