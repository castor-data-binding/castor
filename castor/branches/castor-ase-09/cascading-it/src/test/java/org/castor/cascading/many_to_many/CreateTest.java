...
public class CreateTest extends AbstractTransactionalJUnit4SpringContextTests {
   ...
   // @After
	...
	this.simpleJdbcTemplate.update("DELETE FROM ManyToMany_BookAuthor WHERE 1=1");
	this.simpleJdbcTemplate.update("DELETE FROM ManyToMany_Book WHERE 1=1");
	this.simpleJdbcTemplate.update("DELETE FROM ManyToMany_Author WHERE 1=1");
 ...
 /*   @Test
	...
	// persist author1 and therefore book2 and therefore author2 and therefore book3
	db.begin();
	db.create(author1);
	db.commit();
